...
CRYPTO_10S_BACKOFF_SOURCE_NAME = os.getenv("CRYPTO_10S_BACKOFF_SOURCE_NAME", "crypto-coingecko-10s-backoff")
CRYPTO_10S_BACKOFF_SOURCE_DESCRIPTION = os.getenv(
    "CRYPTO_10S_BACKOFF_SOURCE_DESCRIPTION",
    "High-frequency 10-second crypto snapshot ingestion with retry/backoff.",
)
...
@dag(
    dag_id="crypto_market_ingest_10s_backoff",
    schedule=timedelta(seconds=10),
    start_date=datetime(2025, 1, 1),
    catchup=False,
    max_active_runs=1,
    tags=["crypto", "ingest", "internet", "10s", "backoff"],
)
def crypto_market_ingest_10s_backoff() -> None:
    @task(
        retries=3,
    ...
    def extract() -> dict[str, dict[str, float | int]]:
        ...
        try:
            ...
            # Let Airflow retries handle transient HTTP errors such as 429/5xx.
            raise RuntimeError(f"HTTP error while fetching crypto prices: {exc.code}") from exc
        except URLError as exc:
            raise RuntimeError("Network error while fetching crypto prices") from exc
    ...
    def transform(payload: dict[str, dict[str, float | int]]) -> list[dict[str, object]]:
        ...
        for asset_id in CRYPTO_IDS:
            ...
            if CRYPTO_VS_CURRENCY not in market:
                continue
            ...
            records.append(
                {
                    ...
                    "asset_id": asset_id,
                    "price_usd": float(market[CRYPTO_VS_CURRENCY]),
                    "event_time": _to_iso_utc(last_updated_int),
                    "provider": "coingecko-10s-backoff",
                }
            )
    ...
    def load(records: list[dict[str, object]]) -> int:
        ...
        with psycopg.connect(UDP_DB_URL) as connection:
            with connection.cursor() as cursor:
                cursor.execute(get_source_query, (CRYPTO_10S_BACKOFF_SOURCE_NAME,))
                row = cursor.fetchone()
                ...
                if row is None:
                    ...
                    cursor.execute(
                        insert_source_query,
                        (source_id, CRYPTO_10S_BACKOFF_SOURCE_NAME, CRYPTO_10S_BACKOFF_SOURCE_DESCRIPTION),
                    )
                    cursor.execute(
                        insert_dataset_query,
                        (str(uuid.uuid4()), source_id, f"{CRYPTO_10S_BACKOFF_SOURCE_NAME}-default"),
                    )
                else:
                    source_id = str(row[0])
...
crypto_market_ingest_10s_backoff()
